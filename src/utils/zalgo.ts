...
export const getRandomString = (n = 8) => [...Array(n)].map(() => String.fromCharCode(Math.floor(Math.random() * 65535))).join('');
...
export const getZalgo = (input: string) => {
    ...
    output = str_arr.map(function(a) {
        if(a == " ") return a;
        for(let i = 0, l = getRandom(16);
            i<l;i++){
                const rand = getRandom(3);
            a += chars[rand.toString() as keyof typeof chars][
...
export const generateDescription = () => {
    ...
    const description = sentences.map((sentence, idx) => {
        ...
        return `${sentenceString}${punctuation[getRandomInt(0, punctuation.length - 1)]} `;
    })
